...
static int button_press_cb (GtkWidget *widget, GdkEvent *event, tilda_term *terminal);
...
void handle_gdk_event (G_GNUC_UNUSED GtkWidget *widget,
                       GdkEvent *event,
                       tilda_term *terminal);
...
static int
register_match (VteRegex * regex, gpointer user_data)
{
    ...
    vte_terminal_match_set_cursor_type (VTE_TERMINAL (term->vte_term), tag, GDK_HAND2);
...
static int
button_press_cb (G_GNUC_UNUSED GtkWidget *widget, GdkEvent *event, tilda_term * terminal)
{
                       ...
                       tilda_term *tt)
{
    ...
    hyperlink = vte_terminal_hyperlink_check_event (terminal, (GdkEvent *) event);

    match = vte_terminal_match_check_event (terminal,
                                            (GdkEvent *) event,
                                            &tag);

    TerminalURLFlavor flavor = tilda_match_registry_get_flavor_for_tag (tt->registry, tag);

    link = hyperlink ? hyperlink : match;
    ...
    if (event->type == GDK_BUTTON_PRESS)
    {
        ...
        switch (button_event->button)
        {
            ...
            case 3: /* Right Click */
            {
                GtkWidget * menu = tilda_context_menu_popup (tt->tw, tt, link, flavor);
            ...
            case 1: /* Left Click */
                ...
                spawn_browser_for_match (tt, match, tag);
                break;
            default:
                break;
        }
                              ...
                              TerminalURLFlavor flavor)
{
    ...
    if (match != NULL && (flavor == TILDA_MATCH_FLAVOR_AS_IS ||
